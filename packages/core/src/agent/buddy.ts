...
/**
 * System instructions defining the secondary conversational agent's personality.
 */
...
/**
 * Wraps the primary stream to inject Buddy persona responses during tool execution.
 * * @param userPrompt - Original user input.
 * @param workerStream - Stream from the main technical agent.
 * @param buddyAgent - Agent configured with the Buddy persona.
 */
export async function* withBuddyMode(
    ...
    while (!workerResult.done) {
        ...
        // Trigger buddy response on tool start
        if (chunk.type === "tool_start") {
            ...
            try {
                ...
                for await (const bc of buddyAgent.askStream(prompt, BUDDY_PROMPT, { chatOnly: true })) {
                    if (bc.type === "content") {
                        buddySpoke = true;
                        yield { type: "buddy_content" as const, content: bc.content };
                    }
