...
import { z } from 'zod';
import { handleExecuteAction, ToolCallbacks } from './actions/execute.js';
...
export function createTools(callbacks?: ToolCallbacks): Record<string, Tool> {
    return {
        execute_command: {
            type: 'function',
            ...
            execute: async ({ command }) => {
                return await handleExecuteAction({ command }, callbacks);
            }
        },
